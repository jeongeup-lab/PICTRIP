...
@pytest.mark.asyncio
async def test_404_unknown_spot(db_session: AsyncSession) -> None:
    with pytest.raises(ResourceNotFound):
        await load_spot_detail(db_session, FakeKto(), "ghost-xyz")


@pytest.mark.asyncio
async def test_404_hidden_spot(db_session: AsyncSession) -> None:
    await _insert_spot(db_session, "DT-HID", show_flag=0)
    with pytest.raises(ResourceNotFound):
        await load_spot_detail(db_session, FakeKto(), "DT-HID")


@pytest.mark.asyncio
async def test_fresh_cache_skips_kto(db_session: AsyncSession) -> None:
    ...
    row = await load_spot_detail(db_session, kto, "DT-FRESH")
...
@pytest.mark.asyncio
async def test_cache_miss_fetches_then_caches(db_session: AsyncSession) -> None:
    await _insert_spot(db_session, "DT-MISS")
    kto = FakeKto(_COMMON, _IMAGES)

    row = await load_spot_detail(db_session, kto, "DT-MISS")
    ...
    row2 = await load_spot_detail(db_session, kto2, "DT-MISS")
    assert kto2.calls == 0
...
async def test_stale_cache_refetches(db_session: AsyncSession) -> None:
    ...
    row = await load_spot_detail(db_session, kto, "DT-STALE")
    assert kto.calls == 3
    assert row.detail_status == "fresh"
    assert row.overview == "한라산 정상 풍경"
...
@pytest.mark.asyncio
async def test_kto_failure_no_cache_is_unavailable(db_session: AsyncSession) -> None:
    await _insert_spot(db_session, "DT-NOCACHE")
    kto = FakeKto(fail=True)

    row = await load_spot_detail(db_session, kto, "DT-NOCACHE")
...
@pytest.mark.asyncio
async def test_kto_failure_with_stale_serves_stale(db_session: AsyncSession) -> None:
    ...
    row = await load_spot_detail(db_session, kto, "DT-STALEFAIL")
...
@pytest.mark.asyncio
async def test_region_sigungu_and_moods(db_session: AsyncSession) -> None:
    ...
    await db_session.execute(
        text(
            ...
            "SELECT 'DT-GEO', id, 0.4, 'image' FROM moods WHERE code = 'sea'"
        )
    )
    await db_session.execute(
        text(
            "INSERT INTO spot_moods (content_id, mood_id, confidence, source) "
            "SELECT 'DT-GEO', id, 0.9, 'code' FROM moods WHERE code = 'mountain'"
        )
    )

    row = await load_spot_detail(db_session, FakeKto(), "DT-GEO")
    assert row.region_name is not None
    assert row.sigungu_name == "종로구"
    assert [m.code for m in row.moods] == ["mountain", "sea"]


@pytest.mark.asyncio
async def test_overview_stored_verbatim(db_session: AsyncSession) -> None:
    await _insert_spot(db_session, "DT-VERB")
    verbatim = "한라산\n  <b>정상</b>에서 본 풍경  "
    kto = FakeKto([{"overview": verbatim, "homepage": "", "tel": ""}], [])

    row = await load_spot_detail(db_session, kto, "DT-VERB")
...
@pytest.mark.asyncio
async def test_image_replace_drops_trailing_no_duplicates(db_session: AsyncSession) -> None:
    ...
    row = await load_spot_detail(db_session, kto, "DT-REPL")
...
@pytest.mark.asyncio
async def test_intro_persisted_and_returned(db_session: AsyncSession) -> None:
    await _insert_spot(db_session, "DT-INTRO")
    kto = FakeKto(_COMMON, _IMAGES, _INTRO)

    row = await load_spot_detail(db_session, kto, "DT-INTRO")
    ...
    row2 = await load_spot_detail(db_session, kto2, "DT-INTRO")
    assert kto2.calls == 0
...
@pytest.mark.asyncio
async def test_category_from_lcls3(db_session: AsyncSession) -> None:
    await db_session.execute(
        text(
    ...
    row = await load_spot_detail(db_session, FakeKto(), "DT-CAT")
