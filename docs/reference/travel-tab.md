...
- 칩·추천 질문 없음 — 진입점은 컴포저뿐이다. 답변 뒤에도 팔로업 칩을 두지 않는다.
...
| 입력 상한 | 500자 (`MAX_MESSAGE_CHARS`) — 서버 스키마와 같은 값이라 422를 미리 막는다 |
...
- `delta` — `text` 누적.
- `cards` / `sources` — 턴 필드 교체. `cards`는 라이터보다 먼저 도착한다.
