...
**고른 카드는 `근처 X` 의 기준점이 된다.** 의도의 `aroundOrigin`(앞 결과나
지금 고른 곳 주변을 묻는가)이 켜지고 `context.focusContentId` 가 있으면 그
카드를 원점으로 주변 조회를 돈다 — `그 옆에` 처럼 **이름을 말하지 않아도**
걸린다. `originPlace` 로 이름이 특정되면 그쪽이 먼저다.

**단, 새 지역을 말하면 앵커보다 지역이 이긴다.** 카드를 잡아둔 채 `부산 근처
카페` 라고 하면 앵커가 통영이어도 부산을 찾는다. 앵커는 대화에 오래 남는데
사용자가 새 지역을 말한 순간 그건 낡은 기준점이다. **직전 턴에서 물려온 지역은
해당하지 않는다** — 후속 질문은 조건을 유지하므로 `regionHints` 가 그대로 실려
오고, 그걸 "새로 말했다"로 읽으면 `여기 근처 카페` 가 영영 안 걸린다.

**맛집·카페는 원점이 있어야 찾는다.** 원점 우선순위는 **고른 카드 > 질문 속
장소 > 지역 > 내 좌표** 다. 좌표를 장소보다 먼저 보면 `대천역 근처 맛집` 이
내 위치 맛집이 된다. 넷 다 없을 때만 `맛집·카페는 장소를 하나 골라 주시면…`
이라고 말한다.

- **질문 속 장소**는 `geocode.locate` 가 좌표로 바꾼다 — 관광지 제목 매칭은
  **물어본 이름을 실제로 담을 때만** 채택하고(안 그러면 `대천역` 이 부산
  `대천천` 이 된다), 없으면 네이버 지역검색으로 좌표를 받는다. 네이버 결과
  제목에도 없으면 **포기하고 다음 원점으로** 넘긴다 — `한옥마을` 이 북촌 카페
  좌표로 잡히는 것을 막는다. 이름은 사용자가 말한 대로 쓴다(네이버가
  `타이어뱅크 대천역점` 을 줘도 `대천역 주변 맛집`).
- **지역**은 반경이 아니라 지역 한정 조회다. `정읍 맛집` 은 3km 원이 아니라
  정읍시 안의 음식점이고, `find_candidates(pool_sql=...)` 로 후보 풀 술어를
  갈아끼운다.

음식점은 혼잡도·mood 가 사실상 없어(13,765곳 중 혼잡도 12곳·mood 0곳) 원점
없이 전국에 뿌리면 `content_id` 순 나열이 된다. 거리가 음식점에 정당한 유일한
정렬이고, 거리는 원점을 요구한다.

**일상어는 분류 체계 낱말로 옮긴다.** `사찰` 은 분류명 `사찰문화체험`(3곳)에
걸려 절이 아니라 체험관을 주고 있었다. 진짜 절은 `불교`(1,004곳)다.
`TAXONOMY_SYNONYMS` 가 사찰·절→불교, 교회·성당→기독교, 놀이공원→테마파크,
식물원→수목원, 트레킹→둘레길을 옮긴다. 답변은 사용자 말을
그대로 쓴다(`사찰 조건으로`). `스키장`·`벚꽃`·`단풍` 은 분류 체계에 실제로
없으므로 억지로 옮기지 않는다. `야시장` 도 옮기지 않는다 — `시장` 은 쇼핑(`SH*`)
분류라 여행 후보 풀(`travel_category_sql()`, `HS`/`NA`/`EX`/`VE`)과 교집합이
비어, 제목 검색으로 두는 편이 실제로 `…야시장` 스팟을 찾는다.

**시도 축약명과 구로 쪼개진 시가 조용히 무시됐다.** `regions` 에는 `충청북도`
로 적혀 있어 `충북` 이 안 맞고, `전주` 는 시군구가 완산구·덕진구로 나뉘어
"여러 개라 포기" 로 빠졌다. 축약명 별칭(`충북→충청북` 등 5개)을 더하고, 같은
시도 안에서 이름 앞부분을 공유하는 구들은 그 공통 부분(`전북특별자치도
전주시`)으로 묶는다. `고성`(강원·경남)·`남구`(부산·대구)처럼 진짜 모호한
것은 그대로 포기한다.

**동반자는 장소 종류로 편다.** `아이랑`·`가족이` 는 담을 축이 없어 예전에는
조건이 통째로 비었다. 이제 의도 추출이 `테마파크`·`동물원`·`수족관`·
`어린이공원`·`체험농장` 으로 펴서 넣는다(여행 풀 안 450곳). 다른 유형이나
분위기를 함께 말하면 그쪽이 이긴다 — `아이랑 갈 만한 부산 바다` 는 바다다.
답변 문장이 `테마파크 조건으로` 라고 **해석을 드러내므로** 사용자가 바로
고칠 수 있다. `가족코스`(337곳)는 쓰지 않는다 — `contentTypeId=25` 여행코스라
장소가 아니라 일정이다.

**조건 없는 자유문은 지역을 되묻는다.** `아이랑 갈 만한 곳` 처럼 말은 했는데
걸 축이 하나도 없으면 `어느 지역으로 찾아볼까요?` 로 답한다. `안녕` 같은
`smalltalk` 과는 다른 문장이다 — 전자는 찾고 싶은 사람이고 후자는 아니다.

**앵커 주변이 비어도 에러가 아니다.** 반경 3km 안에 결과가 없으면
`{장소} 주변 3km 안에는 맛집이 없어요` 로 답한다. 한적한 곳을 고를수록 비는데,
한적한 곳을 찾아주는 앱이라 에러 턴으로 끝내면 방향이 반대가 된다.

**상세 값은 KTO 마크업을 걷어내고 싣는다.** `intro` 원문에는 `<br>` 이 섞여
있어 그대로 문장에 넣으면 `주차는 가능<br>요금(무료)예요` 가 된다. 상세 화면이
이미 `htmlToPlainText` 로 하던 일을 서버가 같은 규칙으로 한다. `overview` 는
여기 해당하지 않는다 — 원문 무변형이 원칙이라 태그가 섞이면 아예 쓰지 않는다.
