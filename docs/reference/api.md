...
**맛집·카페는 원점이 있어야 찾는다.** 카테고리가 전부 `FD*` 로 풀리거나 `맛집`·`카페` 같은 일상어가 잡히면(분류 체계에 그 이름이 없어 코드 조회가 실패한다) 좌표나
앵커를 원점으로 주변을 돈다. 둘 다 없으면 목록 대신 `맛집·카페는 장소를 하나
골라 주시면…` 이라고 말한다. 음식점은 혼잡도·mood 가 사실상 없어(13,765곳 중
혼잡도 12곳·mood 0곳) 지역 단위로 뿌리면 `content_id` 순 나열이 된다. 거리가
음식점에 정당한 유일한 정렬이고, 거리는 원점을 요구한다.
