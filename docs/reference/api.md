...
찍힌 단계가 범인이다), 건 조건을 이름으로 되짚는 `answer`(`제주 + 실내 조건으로는
0곳이에요`), 그리고 풀 수 있는 축마다 하나씩 나오는 `refinements`
(`실내 조건 풀기` · `지역 넓히기`, 최대 3개). `intent`는 **실제로 검색에 쓰인
값**을 돌려주므로 이미 떨어져 나간 축은 칩으로 제안하지 않는다 — 실내 재조회가
`categoryKeywords`를 비웠으면 `계곡 조건 풀기`는 나오지 않는다.

`AGENT_NO_RESULTS`는 조건이 아닌 이유로 비었을 때만 남는다 — 지목한 장소를 못
찾음(`place_only`), 앵커 반경 3km에 아무것도 없음, 축제 풀이 0건. 이때는 풀
조건 자체가 없어 제안할 것이 없다.
