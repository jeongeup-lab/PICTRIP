  ...
  const onChipPress = useCallback(
    async (chip: DockChip) => {
      ...
      if (inner.kind === "anchor") {
        if (!focused) return;
        const anchor = { contentId: focused.contentId, action: inner.action };
        startTurn({
          id,
          question: anchorQuestion(focused.title, inner.label),
          request: "",
          photo: null,
          anchor,
        });
...
- [ ] **Step 3: `GlassSheet` 가 아직 쓰이는지 확인한다**

```bash
cd mobile && grep -rn "GlassSheet" src
```

지도 탭(`src/app/map.tsx` 또는 `src/features/map/`)이 쓰고 있으면 **남긴다.**
아무도 안 쓰면 `src/components/GlassSheet.tsx` 와 `src/lib/sheet-snap.ts` 도 지운다.
