...
| 17 | 2026-08-22 | **AI 국외 이전은 별도 동의로 받는다 (공개 경로 폐기)** | 공개 경로(법 제28조의8 ①3호)는 `처리위탁` 에만 열린다. 그런데 DeepSeek 오픈플랫폼은 로그인 시 `deepseek-privacy-policy.html` 에 동의시키고, 그 방침이 "당사 기술 학습∙개선 목적으로 추가적으로 이용할 수 있습니다" 라고 적는다 — 수탁자가 자기 목적으로 처리하면 위탁이 아니다. **API 콘솔을 직접 열어 확인했으나 학습 거부 토글이 없다** (메뉴는 Usage·API keys·Top up·Billing·Docs·Help·Pricing 뿐, `/settings`·`/account` 는 `/usage` 로 리다이렉트). 챗 앱 토글은 별도 약관·별도 로그인이라 API 트래픽에 닿지 않는다 | `consent/components/AiTransferSheet.tsx` · `PUT /users/me/consents/ai-transfer` |
...
- LLM 공급자는 `LLM_PROVIDER` 로 고른다(`gemini` 기본 · `deepseek` · 로컬 `codex`).
  LLM 429 는 여행 탭이 실제로 맞는 한계라 대체 경로를 열어 뒀다.
