...
- 턴 하나가 Gemini 왕복 1회다. 전체 266턴이면 약 20분, `agent_ask` 레이트리밋
...
| 결과 | `expect_spots` (`some`·`none`·`any`) | 목록이 나와야 하는 턴인가. 음식점처럼 제목 근거가 필요한 케이스는 반환된 카드 제목도 검사하며, `any`는 정직한 0곳 결과를 허용한다 |
...
| B 검색 | 34 | 지역 12종 표기 + 카테고리·분위기·혼잡도·실내·근처·축제·장소명 축 |
...
| F 데이터 경계 | 23 | 후보 풀에 없는 것, 모호·동명 지역, 읍면동, 모순 조건 |
...
| J 결과 품질 | 24 | **개수가 아니라 내용을 본다** — 요청 지역 안인가, 이미지·좌표가 있나, 중복인가, `근처` 가 정말 근처인가 |
| K 지역 대체 | 10 | 그 지역에 그 조건이 없을 때(서울 바다·대전 해수욕장) 어떻게 답하나 |

`J` 는 `expect_region`·`expect_images`·`expect_placed`·`expect_unique`·
`expect_within_km` 로 **응답 카드 하나하나를 검사한다.** 라우팅만 보던 층으로는
`충북 동굴` 이 강원 동굴을 주는 것도, `경남 사찰` 이 체험관을 주는 것도 잡히지
않았다.
...
이 붙은 턴은 그 카드를 앵커로 잡는다. 앵커는 앱에서 캐러셀로 보고 있는 카드에
해당하고 **다음 턴까지 유지된다** — 0곳 턴이 끼어도 잃지 않는다.

여기서만 잡히는 것: 카드를 고른 뒤 `여기 근처 카페` 가 주변 조회로 가는가,
8턴 뒤에도 조건이 남는가, 못 하는 요구가 껴도 대화가 이어지는가.

음식점 질의는 특히 제목을 촘촘히 본다 — 삼겹살·국밥처럼 요청한 음식이 있을 때는
반환된 카드 제목에 그 말이 실제로 포함되는지 확인한다. 결과가 0곳이면 억지로
장소를 만들지 않고, 조건에 맞는 결과가 없다는 답을 통과시킨다.
...
| ID | 입력 | 지금 | 왜 남겨두나 |
|---|---|---|---|
| M8.4 | `그 옆에` | 지역 되묻기 | 카드를 잡아둔 상태에서도 지시어 하나로는 `aroundOrigin` 이 안 켜진다(실측). 더 밀어붙이면 `그거`(그 장소 자체)·`거기 말고`(제외) 까지 주변 조회로 끌려간다 |
