...
사전계산은 `scripts.precompute_matches` 가 **한 트랜잭션에 전수를 다시 쓴다**
(실측 2,573건 32초). 부분 커밋이 보이면 아래 빈-테이블 예외가 풀려 아직 못 채운
게시물이 전부 걸러지므로, 중간 상태가 밖으로 보이면 안 된다. TRUNCATE 가 아니라
DELETE 인 것도 락 때문이다 — 읽는 쪽은 MVCC 스냅샷으로 지나간다. 저장하는 건
...
피드는 매칭 3칸이 다 차는 게시물만 올린다. 후보 선정도 `_MATCHES_SQL` 과 **같은
라이브 조인으로 센다** — 사전계산 행은 남아 있는데 스팟이 숨겨지거나 대표사진이
바뀐 구간이 실제로 있고(일일 동기화 뒤 재계산 전), 카운트를 사전계산 쪽에서만
보면 그 구간에 타일이 1~2칸만 찬 슬라이드가 나간다. 사전계산 테이블이
